...
for seed in tqdm(SEED_LIST[:10], desc='seed loop'):
    ...
    model = Turku(len(word_to_ix), len(pos_to_ix), len(
        dependency_to_ix), pretrained_embedding_matrix, hidden_dim=128)
    ...
    train_out = train_model(model, dataloaders['train'], dataloaders['dev'], word_to_ix,
                            ...
                            lr_scheduler=None,
                            num_epochs=4, early_stopped_patience=0, batch_size=batch_size,
                            verbose=True)

    (model, train_f1, val_f1, history) = train_out

    y_true, y_pred, predictions = predict_model(model, dataloaders['test'],
                                                word_to_ix, pos_to_ix, distance_to_ix,
                                                dependency_to_ix, char_to_ix, batch_size,
                                                optimizer_ft)
