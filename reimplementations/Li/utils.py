        ...
        for doc in docs:
            ...
            for sentence in sentences:
                ...
                for dependency in dependencies:
                    ...
                    # if dependency.getAttribute("type") == 'conj_and':
                    #     continue
                ...
                if len(start_end_list) > 0:
                    print(f"{'-'*20}{document_idx}:{sentence.getAttribute('id')}{'-'*20}")
                ...
                for entity_idx, ((start_token, end_token), (start_entity, end_entity)) in enumerate(start_end_list):
                    ...
                    if shortest_path:
                        ...
                        continue
                        # Fill with full-sentence
#                         for idx, token in enumerate(token_list):
#                             if token['id'] == start_token:
#                                 # "entity_1"
#                                 try:
#                                     synonym = synonym_dict[entity_idx_to_text[start_entity].lower()]
#                                 except KeyError:
#                                     tkn = entity_idx_to_text[start_entity].lower()
#                                 else:
#                                     # print("[SYNONYM]: Replace `{}` with `{}`".format(token['text'].lower(), synonym))
#                                     tkn = synonym
#                                 shortest_token_list.append(tkn)
...
#                                 # "entity_2"
#                                 try:
#                                     synonym = synonym_dict[entity_idx_to_text[end_entity].lower()]
#                                 except KeyError:
#                                     tkn = entity_idx_to_text[end_entity].lower()
#                                 else:
#                                     # print("[SYNONYM]: Replace `{}` with `{}`".format(token['text'].lower(), synonym))
#                                     tkn = synonym
#                                 shortest_token_list.append(tkn)
...
#                             shortest_dep_list.append(dep['type'])
                    ...
                    if label_pairs[entity_idx] == 1:
                            print(f"TRUE:\n SHORTEST: {' | '.join(shortest_token_list)}")
                            print(f" FULL: {' '.join(full_token_list)}")
                    
                    if label_pairs[entity_idx] == 0:
                            print(f"FALSE:\n SHORTEST: {' | '.join(shortest_token_list)}")
                            print(f" FULL: {' '.join(full_token_list)}")
